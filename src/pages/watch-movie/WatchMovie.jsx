...
const WatchMovie = () => {
    ...
    console.log(info)
    const background = apiConfig.originalImage(info.poster_path || info.backdrop_path);
    ...
    return (
        <Title title={`${info.title || info.name} | Watch`}>
            <div className='watch-movie' style={{ backgroundImage: `url(${background})` }}>
                <div className='container'>
                    <div className='watch-movie__wrap section mb-3'>
                        ...
                        <div className='watch-movie__wrap__info section '>
                            <h1 className='watch-movie__wrap__info__name'>{info.title || info.name}</h1>
                            <p className='watch-movie__wrap__info__overview'>{info.overview}</p>
                            <p className='watch-movie__wrap__info__release-date'>Release Date: {info.release_date}</p>
                            <p className='watch-movie__wrap__info__vote-average'>{info.vote_average}</p>
                        </div>
                    </div>
